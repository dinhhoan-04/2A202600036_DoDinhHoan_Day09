...
**Bằng chứng:** Comment `# NXTung — Sprint 1` ở đầu `supervisor_node()` trong `graph.py`.
...
Thiết kế `AgentState` TypedDict đầy đủ từ đầu (15 fields) giúp các thành viên khác biết rõ họ được đọc/ghi field nào, không cần hỏi lại. Đặc biệt field `workers_called` và `history` giúp Nguyễn Công Thành viết `analyze_traces()` mà không cần đọc implementation chi tiết của từng worker.
...
Toàn bộ pipeline bị block nếu `graph.py` chưa xong. Trần Quốc Khánh cần biết `state["needs_tool"]` có được set không trước khi implement policy_tool.py; Nguyễn Công Thành cần `run_graph()` function signature để viết eval_trace.py.

**Phần tôi phụ thuộc vào thành viên khác:**

Cần Nguyễn Viết Hùng, Đỗ Đình Hoàn, và Trần Quốc Khánh implement `run(state) -> state` interface đúng. Nếu signature thay đổi, `build_graph()` của tôi break ngay.
